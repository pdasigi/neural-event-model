...
class NEM:
    ...
    def train_nem(self, inputs, labels, pretrained_embedding_file=None):
        '''
        ...
        if self.use_event_structure:
            model = self._build_structured_model(inputs, pretrained_embedding)
        else:
            model = self._build_flat_model(inputs, pretrained_embedding)
        model.summary()
        model.compile("adam", "categorical_crossentropy", metrics=["accuracy"])
        model.fit(inputs, labels, epochs=20, validation_split=0.1)
        self.model = model
    ...
    def test_nem(self, inputs, labels):
        ...
        '''
        self.model.evaluate(inputs, labels)

    def _build_structured_model(self, inputs, pretrained_embedding=None) -> Model:
    ...
    def _build_flat_model(self, inputs, pretrained_embedding=None) -> Model:
...
def main():
    ...
    if args.train_file is not None:
        ...
        nem.train_nem(train_inputs, train_labels, args.embedding_file)
